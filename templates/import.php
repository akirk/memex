...
<header class="memex-page-header">
	<h1><?php esc_html_e( 'Import notes', 'memex' ); ?></h1>
...
<?php if ( $result ) : ?>
	<div class="memex-notice memex-notice-success">
		<p>
			<?php
			printf(
...
<?php elseif ( 'no-file' === $error ) : ?>
	<div class="memex-notice memex-notice-error"><p><?php esc_html_e( 'Please select a file to import.', 'memex' ); ?></p></div>
<?php elseif ( 'unknown-type' === $error ) : ?>
	<div class="memex-notice memex-notice-error"><p><?php esc_html_e( 'Could not detect the file type. Please pick one explicitly below.', 'memex' ); ?></p></div>
<?php endif; ?>

<form class="memex-import-form" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
	<input type="hidden" name="action" value="memex_import">
	<?php wp_nonce_field( 'memex_import' ); ?>

	<div class="memex-import-types">
	...
	<div class="memex-upload">
		<input type="file" name="import_file" accept=".zip,.enex,.xml,.json,.md,.markdown,.txt" required>
		<p class="memex-muted">
			<?php
			printf(
